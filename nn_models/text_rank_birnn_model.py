...
class TextRankBiRnnModel(object):
  def __init__(self, tensor_dict, config, opt=None):
    ...
    with tf.variable_scope('sentence_embedding'):
      query_embed = nn_layers.birnn_text_embedding(self.query_inputs, self.vocab_size, self.embed_size, self.rnn_size, self.num_rnn_layers, config.rnn_type, config.pad_id, combiner='max')
      tf.get_variable_scope().reuse_variables()
      title_pos_embed = nn_layers.birnn_text_embedding(self.title_inputs_pos, self.vocab_size, self.embed_size, self.rnn_size, self.num_rnn_layers, config.rnn_type, config.pad_id, combiner='max')
      title_neg_embed = nn_layers.birnn_text_embedding(self.title_inputs_neg, self.vocab_size, self.embed_size, self.rnn_size, self.num_rnn_layers, config.rnn_type, config.pad_id, combiner='max')
    ...
    with tf.name_scope('score'):
      ...
      #self.score = tf.reduce_mean(tf.cast(tf.greater(predictions, 0.5), tf.float32))

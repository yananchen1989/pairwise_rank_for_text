...
class TextRankDnnModel(object):
  def __init__(self, tensor_dict, config, opt=None):
    ...
    with tf.variable_scope('sentence_embedding'):
      query_embed = nn_layers.sparse_text_embedding(self.query_inputs, [self.vocab_size, self.embed_size])
      tf.get_variable_scope().reuse_variables()
      title_pos_embed = nn_layers.sparse_text_embedding(self.title_inputs_pos, [self.vocab_size, self.embed_size])
      title_neg_embed = nn_layers.sparse_text_embedding(self.title_inputs_neg, [self.vocab_size, self.embed_size])
